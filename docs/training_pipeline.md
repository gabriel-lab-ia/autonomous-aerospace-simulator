...
The first training stage uses synthetic aerospace state/sensor samples. The
pipeline can also consume prepared telemetry generated by the reproducible
...
Simulator-derived imitation data is generated with:
...
This data comes from the simplified simulator, not real aerospace flights.
...
2. Tune and stress-test the PID baseline.
3. Expand trajectory-based imitation datasets across scenarios.
4. Compare fixed, heuristic, PID, and neural controllers across scenarios.
